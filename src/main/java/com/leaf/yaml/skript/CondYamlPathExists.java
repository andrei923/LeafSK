	...
	public boolean check(final Event event) {
		return path.check(event, new Checker<String>() {
			...
			public boolean check(final String s) {
				if (!Leaf.YAML_STORE.containsKey(file.getSingle(event)))
					return false;
				if (path.isSingle())
					return Leaf.YAML_STORE.get(file.getSingle(event)).getAllKeys().contains(path.getSingle(event));
				else {
					boolean check;
					for (String p : path.getAll(event)) {
						check = Leaf.YAML_STORE.get(file.getSingle(event)).getAllKeys().contains(p);
						if (!check) {
							return false;
						}
					}
